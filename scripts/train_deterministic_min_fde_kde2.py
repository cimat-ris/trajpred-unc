...
from utils.calibration import generate_uncertainty_evaluation_dataset, regression_isotonic_fit, calibrate_and_test, gaussian_kde_from_gaussianmixture
...
def main():
	...
	minfdes = []
	for k in tqdm.tqdm(range(tpred_samples_test.shape[1])):
		xs = []
		ys = []
		#tpred = tpred_samples[:,k,11,:].reshape((-1,2,1))
		gt    = target_test[k,11,:].numpy().reshape((1,2))
		# Prediction samples
		world_samples   = tpred_samples_test[:,k,11,:].reshape((-1,2))
		sigmas_prediction = sigmas_samples_full[:,k,11,:]
		
		#print(world_samples.shape)
		#print(gt.shape)
		...
		if world_samples.shape[0]>0:
			...
			kde, sub_samples = gaussian_kde_from_gaussianmixture(world_samples, sigmas_prediction, resample_size=20)
	...
	for k in tqdm.tqdm(range(tpred_samples_test.shape[1])):
		...
		if world_samples.shape[0]>0:
			# Build a Kernel Density Estimator with these samples
			#kde         = st.gaussian_kde(world_samples.T)
			kde, sub_samples0 = gaussian_kde_from_gaussianmixture(world_samples, sigmas_prediction, resample_size=20)
			#print("--")
			#print(sub_samples0)
			#print("--")
			...
			alphas_samples, fs_samples, sorted_samples = samples_to_alphas(kde, sub_samples0.T) ##
			...
			kde               = st.gaussian_kde(sub_samples0.T,weights=importance_weights) ###
			sub_samples = kde.resample(20).T #.T.reshape((-1,2,1))
			#print(sub_samples)
